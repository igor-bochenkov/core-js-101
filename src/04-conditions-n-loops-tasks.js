...
function isBracketsBalanced(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    ...
    const OPEN_BR_INDEX = BRACKETS_OPEN.findIndex((item) => item === currentSymbol);

    if (stack.length === 0) {
      stack.push(OPEN_BR_INDEX);
    } else {
      const topElement = stack[stack.length - 1];

      if (currentSymbol === BRACKETS_CLOSE[topElement]) {
        stack.pop();
      } else if (OPEN_BR_INDEX !== -1) {
        stack.push(OPEN_BR_INDEX);
      }
    }
  }
